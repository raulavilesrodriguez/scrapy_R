...
while (TRUE) {
  result <- try({
    ...
    if (status_code(resp) != 200) stop("No 200")
    
    dest_page <- paste0(route_directory, '/page-',page_id,'.html')
    writeBin(content(resp, "raw"), dest_page)
    
    Sys.sleep(runif(1, 4, 8))  # PAUSA CLAVE
...
#-----PRUEBA LINK REAL-----
download.file(
  "https://www.plusvalia.com/venta/inmuebles/pichincha/quito?page=1",
  "test.html",
  quiet = TRUE
)
